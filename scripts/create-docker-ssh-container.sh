...
docker build -t friday-ubuntu-ssh . # build the image

rm Dockerfile # remove the created Dockerfile

container_name="friday-ssh-container"

docker run -d -p 2222:22 --name $container_name friday-ubuntu-ssh # run the container
