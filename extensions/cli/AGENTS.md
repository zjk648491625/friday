...
This is a CLI tool for Friday AI that provides an interactive AI-assisted development experience. The architecture consists of:
...
3. **Friday SDK Integration** (`src/fridaySDK.ts`): Initializes the Friday SDK client with:
...
7. **Hooks System** (`src/hooks/`): Event interception system for extending CLI behavior
   ...
   - **Config locations** (lowest to highest precedence):
     - `~/.claude/settings.json`, `~/.friday/settings.json` (user-global)
     - `.claude/settings.json`, `.friday/settings.json` (project)
     - `.claude/settings.local.json`, `.friday/settings.local.json` (project-local)
