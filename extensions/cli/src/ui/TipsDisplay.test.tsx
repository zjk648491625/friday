...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * FRIDAY_CLI_TIPS.length);
    const expectedTip = FRIDAY_CLI_TIPS[expectedTipIndex];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = FRIDAY_CLI_TIPS.some(
...
describe("FRIDAY_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(FRIDAY_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    FRIDAY_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = FRIDAY_CLI_TIPS.join(" ").toLowerCase();
