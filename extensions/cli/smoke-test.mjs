...
runTest("Local packages are bundled", () => {
  ...
  // Check for code from @friday-ai/config-yaml
  if (
    !bundleContent.includes("AssistantUnrolled") &&
    !bundleContent.includes("config-yaml")
  ) {
    throw new Error("@friday-ai/config-yaml not properly bundled");
  }

  // Check for code from @friday-ai/openai-adapters
  ...
  if (
    ...
    throw new Error("@friday-ai/openai-adapters not properly bundled");
  }
});
