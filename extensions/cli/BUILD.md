...
The Friday CLI uses esbuild to bundle the application along with local packages (`@friday-ai/config-yaml` and `@friday-ai/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
