...
process.env.FRIDAY_CLI_ENABLE_TELEMETRY = "0";
process.env.FRIDAY_ALLOW_ANONYMOUS_TELEMETRY = "0";
...
global.fetch = vi
  ...
  .mockImplementation(async (url: string | URL, ...args) => {
    ...
    if (urlString.includes("get-assistant/friday-ai/default-cli-config")) {
      return {
        ok: true,
        status: 200,
        json: async () => ({
...
// NOTE: FRIDAY_GLOBAL_DIR is set to a unique per-worker temp dir in
