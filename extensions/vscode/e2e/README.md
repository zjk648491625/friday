...
- This depends on the model provider written inside the `test-friday/config.json` and `test-friday/config.yaml`. If the model provider is `mock`, it will use `MockLLM`, and if it uses `test`, it will use `TestLLM`.
