...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by friday
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while friday makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "friday.diffVisible", true);
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Friday
      // Before adding this back, we need to distinguish between human and Friday
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand("friday.rejectDiff");
      }
