...
export class VsCodeMessenger {
  ...
  constructor(
    ...
    // Friday AI: Session sharing removed (local-only mode)
    this.onWebview("session/share", async (_msg) => {
      // No-op: sharing disabled in local-only mode
    });
    ...
    this.onWebviewOrCore("isTelemetryEnabled", async (_msg) => {
      return false;
    });

    this.onWebviewOrCore("getUniqueId", async (_msg) => {
      return "LOCAL_ONLY";
    });
