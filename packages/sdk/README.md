...
This SDK provides programmatic access to Friday's Hub APIs and functionality.

## Overview

The Friday SDK is structured into separate language-specific packages, currently with implementations for TypeScript and Python, with more languages planned for the future:
...
1. OpenAPI-generated clients for the Friday Hub API
2. A wrapper layer that exposes a `Friday.from()` method to easily initialize and use Friday assistants with an OpenAI-compatible interface

## For End Users

End users should install the published packages directly:

- For TypeScript/JavaScript: `npm install @friday-ai/sdk`
- For Python: `pip install friday-ai`
