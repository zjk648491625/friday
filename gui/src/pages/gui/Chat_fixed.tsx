...
const StepsDiv = styled.div`
  ...
  /* AI message row 鈥?tight blocks, avatar only on first of a sequence */
  .msg-row {
    display: flex;
  ...
  /* User message row 鈥?right side, auto-width */
  .msg-row.user {
  ...
  const renderChatHistoryItem = useCallback(
    (item: ChatHistoryItemWithMessageId, index: number) => {
      ...
      if (message.role === "assistant") {
        ...
        // Skip tool messages when checking previous item for avatar grouping.
          ...
          <div className={`msg-row ${showAvatar ? "with-avatar" : "no-avatar"}`}>
            {showAvatar && (
              <div className="msg-avatar-col">
                <div className="msg-avatar-icon ai" title="Friday AI">馃</div>
  ...
  return (
    <>
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        {/* Scroll navigation buttons 鈥?floating emoji */}
        <style>{`
          ...
          .history-popup { position:absolute;right:40px;top:0;width:280px;max-height:400px;
            overflow-y:auto;background:var(--vscode-sideBar-background, #1e1e1e);
            border:1px solid var(--vscode-panel-border, #333);border-radius:8px;
            box-shadow:0 4px 20px rgba(0,0,0,0.4);z-index:50;padding:8px 0; }
