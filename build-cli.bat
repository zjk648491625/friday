...
:: Pre-link config-yaml (removed from deps to avoid npm file: bug on Windows)
...
:: Pre-create junctions for all local packages (avoids npm file: resolution bugs on Windows)
...
:: npm 10.x file: dep bug on Windows -> clean install with minimal package.json
...
if not exist "node_modules\esbuild\package.json" (
    ...
    echo {"name":"tmp","type":"module","dependencies":{"esbuild":"^0.25.9","ink":"^6","react":"^19","winston":"^3","node-machine-id":"^1","@opentelemetry/api":"^1","dotenv":"^16","express":"^5","chalk":"^5","commander":"^14","cors":"^2","strip-ansi":"^7","uuid":"^9","yaml":"^2","date-fns":"^4","fkill":"^8","swr":"^2","glob":"^11","handlebars":"^4","turndown":"^7","open":"^10","execa":"^9","gradient-string":"^3","nock":"^14","@modelcontextprotocol/sdk":"^1"}} > package.json
